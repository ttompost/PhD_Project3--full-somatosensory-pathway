...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul  8 16:19:35 2024
...
folder = '/Users/teatompos/Documents/GitHub/StimulusRepInAscendingPathway/activity_per_amplitude/'
for amplitude in range(0, 95, 5):
    ...
    filename1 = f'/Users/teatompos/Documents/GitHub/StimulusRepInAscendingPathway/activity_per_amplitude/amplitude_{amplitude}_id.csv'
    filename2 = f'/Users/teatompos/Documents/GitHub/StimulusRepInAscendingPathway/activity_per_amplitude/amplitude_{amplitude}_time.csv'

    NeuronID = pd.read_csv(filename1, header=None)
    NeuronID.columns = ["NeuronID"]
    Spikes = pd.read_csv(filename2, header=None)
    Spikes.columns = ["Spike_times"]
    ...
    GeneratedSpikes = SpikeGeneratorGroup(BrainstemNeurons,  # number of spiking sources
          indices=np.array(NeuronID["NeuronID"]), # which neurons will make each spike
          times=np.array(Spikes["Spike_times"]*1000)*ms) # spike times for each neuron
    ...
    np.savetxt(folder + 'VPM/amplitude_' + str(amplitude) +'_id.csv', np.asarray(VPM_spikes.i), delimiter=',')
    np.savetxt(folder + 'VPM/amplitude_' + str(amplitude) +'_time.csv', np.asarray(VPM_spikes.t), delimiter=',')
    np.savetxt(folder + 'L4E/amplitude_' + str(amplitude) +'_id.csv', np.asarray(L4E_spikes.i), delimiter=',')
    np.savetxt(folder + 'L4E/amplitude_' + str(amplitude) +'_time.csv', np.asarray(L4E_spikes.t), delimiter=',')
    np.savetxt(folder + 'L4I/amplitude_' + str(amplitude) +'_id.csv', np.asarray(L4I_spikes.i), delimiter=',')
    np.savetxt(folder + 'L4I/amplitude_' + str(amplitude) +'_time.csv', np.asarray(L4I_spikes.t), delimiter=',')
